...
const FormSection = () => {
  ...
  return (
    <>
      <Formik onSubmit={sendForm} initialValues={{ name: '', email: '', phone: '' }}>
        {({ handleSubmit }) => (
          <form onSubmit={handleSubmit} className={style.form}>
            <span className="title_label">LET’S GET STARTED</span>
            <h2 className={style.h2}>Get A Free Consultation.</h2>
            <p>
              The best ideas start from the brif. Let us to know what you need and we will realise it in sucsessfull product.
            </p>
            <div className={style.form_items}>
              <div className={style.fields_wrap}>
                <Field name="name" placeholder="Name" type="text" component={InputField} required />
                <Field name="email" placeholder="Email" type="email" component={InputField} required />
                <Field name="phone" placeholder="Phone Number" type="tel" component={InputField} required />
              </div>
      ...
      <FormModal onCloseFormModal={onCloseFormModal} openFormModal={openFormModal} />
    </>
  );
};
